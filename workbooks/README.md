...
- **Empty-state messages** — each tile has a `noDataMessage` property that controls the text shown when its query returns no rows (e.g. "No Windows servers found in scope. Adjust the Subscriptions or Resource groups filters."). Edit these in the tile's query settings, or directly in [server-patch-inventory-compliance.workbook.json](server-patch-inventory-compliance.workbook.json), to tailor the guidance for your environment.
